package br.com.edsonajeje.cadatroAPI.controllers;

import br.com.edsonajeje.cadatroAPI.model.entities.Client;
import br.com.edsonajeje.cadatroAPI.model.entities.ClientResponse;
import br.com.edsonajeje.cadatroAPI.model.repositories.ClientRepository;
...
public class ClientController {
    ...
    public ResponseEntity<ClientResponse> createClient(@Valid @RequestBody Client client) {
        ...
        if(cpf.isPresent() || email.isPresent()){
            if (cpf.isPresent() && email.isPresent()) {
                response.setMessage("E-mail is required and should be unique. " +
                        "CPF is required and should be unique");
            } else if(email.isEmpty()) {
                response.setMessage("CPF is required and should be unique");
            } else {
                response.setMessage("E-mail is required and should be unique.");
            }
    ...
    public ResponseEntity<ClientResponse> updateClient(@RequestBody Client client, @PathVariable int id) {
        ...
        if(data){
            ...
            response.setMessage("Client id: "+id+" shouldn't be found");
